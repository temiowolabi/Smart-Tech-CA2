...
#datadir = ("C:\\Users\\madok\\Desktop\\driving_log.csv")
...
def balance_steering_angle():
    ...
    max_samples = max(angle_count.values())

    balanced_data = []

    for angle, count in angle_count.items():
        data_points = data[data['steering'] == angle]
        balanced_data += data_points.sample(n=max_samples, replace=True).values.tolist()
